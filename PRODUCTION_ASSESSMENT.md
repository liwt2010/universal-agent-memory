...
| 维度 | 实测数字 | 数据来源 |
|------|----------|----------|
| 代码行数 | **~12,200 行**(代码非空 `.py/.yml/.json/.toml`) | 2026-07-10 脚本统计 |
| 代码文件 | **77 个** | 同上 |
| 测试用例 | **346 个**(21 个本地 skipped:无 PG/Redis/Neo4j 服务;CI service container 全跑通) | `python -m unittest discover -s tests` |
...
| **当前(7-10 v4)** | **~12,200** | **346** | **6(全 6 真实 CI)** | **B+/A-** | **+ 跨层 cascade forget + 29 测试 + GDPR JSONL audit** |
...
| **总计** | **346**(21 skipped 本地) | **6/6 后端全部 CI 真实 e2e + cascade ✅** |
...
| **测试覆盖** | **A** | **346 测试**(v4 +29 cascade),**6/6 后端真实验证**(7-10 v3),mock 测试只占 ~83%;5 套真实 e2e(PG/Chroma/Redis/Neo4j/InMemory)+ 1 套真实 unit(JSONB)+ cascade audit | 真实 LLM 抽样测试 ~5 个(占比 <2%);100k 高并发压测未跑 |
...
**剩余 6 步**(v4 完成后):1, 2, 3, 4, 5, 7 —— 其中 4 复用刚打好的 6 后端真实 CI 基础,7 是单点压测,1 + 2 + 3 + 5 都需要外部资源。
...
**这是 v1 production 路径上又走一大步** —— 6/6 后端 CI 真实验证是 7 步里**唯一基础设施闭环**的那一步(run #21 + #24),**cascade forget 是 v4 路线补的唯一产品能力**(剩下都是规模化验证)。建议 v4 完成后的下一步优先级:
